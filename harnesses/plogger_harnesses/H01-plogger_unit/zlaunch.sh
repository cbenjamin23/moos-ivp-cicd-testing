...
run_case() {
    ...
    echo "case=$case_name  case_result=$status  expected=$EXPECTED  actual=$actual  artifact=$artifact_status  $line" >> "$RESULTS_FILE"
    cd "$HARNESS_DIR"
}
...
run_case_isolated() {
    ...
    echo "case=$case_name  case_result=$status  expected=$EXPECTED  actual=$actual  artifact=$artifact_status  $line" > "$case_result_file"
