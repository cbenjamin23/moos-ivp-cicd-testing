...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H01-collision_behavior_motion
...
get_case_config() {
    ...
    case "$CASE_NAME" in
        ...
        no_alert_request_absent_pass)
            ...
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/no-alert-request-absent-pass-vehicle.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/no-alert-request-absent-pass-vehicle.xbhv"
            ;;
        post_per_contact_info_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/post-per-contact-info-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/post-per-contact-info-pass-vehicle.xbhv"
            ;;
        behavior_filter_absent_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/behavior-filter-absent-pass-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/behavior-filter-absent-pass-vehicle.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/behavior-filter-absent-pass-vehicle.xbhv"
            ;;
        head_on_resolve_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/head-on-resolve-pass-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/head-on-resolve-pass-vehicle.xmoos"
            ;;
        pwt_outer_too_small_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/pwt-outer-too-small-fail-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/pwt-outer-inactive-pass-vehicle.xbhv"
            ;;
        pwt_grade_quadratic_pass)
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/pwt-grade-quadratic-pass-vehicle.xbhv"
            ;;
        pwt_grade_quasi_pass)
        ...
        use_refinery_pass)
            ...
            ;;
        contact_type_required_absent_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/contact-type-required-absent-pass-shoreside.xmoos"
        ...
        bad_collision_depth_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-quick-fail-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/helm-malconfig-fail-vehicle.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-collision-depth-fail-vehicle.xbhv"
            ;;
        *)
