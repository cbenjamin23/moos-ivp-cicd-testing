...
run_case_isolated() {
    ...
    (
        ...
        if [ "$JUST_MAKE" != "yes" ] && [ "$launch_rc" = 0 ]; then
            uMayFinish --alias="uMayFinish_h02_${case_idx}" --max_time=${MAX_TIME} targ_shoreside.moos
            launch_rc=$?
...
if [ "$CASE" != "" ]; then
    ...
    CASES="head_on_thresh_below_pass head_on_thresh_edge_pass head_on_thresh_above_pass head_on_thresh_below_mirror_pass head_on_thresh_edge_mirror_pass head_on_thresh_above_mirror_pass"
fi
