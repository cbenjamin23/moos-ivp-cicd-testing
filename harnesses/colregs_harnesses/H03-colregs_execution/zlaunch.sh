...
run_case() {
    ...
    emit_case_row "$case_name" "$status" "$EXPECTED" "$actual" "$line" "$wall_note" >> "$RESULTS_FILE"
    stop_mission_apps "$MISSION_DIR"
    clear_xfiles
    cd "$HARNESS_DIR"
}
...
run_case_isolated() {
    ...
    emit_case_row "$case_name" "$status" "$EXPECTED" "$actual" "$line" "$wall_note" "launch_rc=$launch_rc" > "$case_row_file"
