...
HARNESS_DIR="${PWD}"
MISSION_DIR="${HARNESS_DIR%/harnesses/H02-cmgr_multi}/missions/cmgr_multi"
...
#------------------------------------------------------------
make_backups() {
    TMP_DIR=`mktemp -d /tmp/cmgr_multi_harness.XXXXXX`
...
get_case_config() {
    ...
    if [ "$CASE_NAME" = "count_two_pass" ]; then
        EXPECTED="pass"
...
CASES="count_two_pass list_alpha_bravo_pass closest_alpha_pass closest_bravo_pass bravo_far_count_one_pass late_bravo_count_two_pass stale_bravo_drop_pass alpha_far_bravo_only_pass"
if [ "$CASE" != "" ]; then
    CASES="$CASE"
fi
