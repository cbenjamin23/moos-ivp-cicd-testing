...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H01-constant_depth_motion
...
Examples:
  ./$ME
  ./$ME --case=constant_depth_hold_pass
  ./$ME --jobs=4 --port_base=15000 --port_stride=12
  ./$ME --just_make --case=constant_depth_bad_depth_fail
EOF
}
...
get_case_config() {
    ...
    if [ "$CASE_NAME" = "constant_depth_hold_pass" ]; then
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-surface-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/constant-depth-surface-pass-vehicle.xbhv"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-update-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-update-pass-vehicle.xmoos"
    elif [ "$CASE_NAME" = "constant_depth_bad_update_preserve_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-bad-update-preserve-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-bad-update-preserve-pass-vehicle.xmoos"
    elif [ "$CASE_NAME" = "constant_depth_duration_complete_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-duration-complete-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/constant-depth-duration-complete-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "constant_depth_bad_depth_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-bad-depth-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/constant-depth-bad-depth-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "constant_depth_bad_peakwidth_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-bad-peakwidth-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/constant-depth-bad-peakwidth-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "constant_depth_bad_basewidth_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-bad-basewidth-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/constant-depth-bad-basewidth-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "constant_depth_bad_summitdelta_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-bad-summitdelta-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/constant-depth-bad-summitdelta-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "constant_depth_bad_mismatch_var_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-bad-mismatch-var-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/constant-depth-bad-mismatch-var-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "constant_depth_missing_duration_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-missing-duration-fail-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/constant-depth-missing-duration-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "constant_depth_missing_nav_depth_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-missing-nav-depth-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-missing-nav-depth-fail-vehicle.xmoos"
    elif [ "$CASE_NAME" = "constant_depth_domain_missing_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-domain-missing-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-domain-missing-fail-vehicle.xmoos"
    elif [ "$CASE_NAME" = "constant_depth_low_elevator_authority_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-low-elevator-authority-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-low-elevator-authority-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/constant-depth-low-elevator-authority-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "constant_depth_control_disabled_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/constant-depth-control-disabled-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/constant-depth-control-disabled-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/constant-depth-control-disabled-fail-vehicle.xbhv"
    else
