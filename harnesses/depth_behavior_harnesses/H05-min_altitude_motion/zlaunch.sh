...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H05-min_altitude_motion
...
Examples:
  ./$ME
  ./$ME --case=min_altitude_guard_pass
  ./$ME --jobs=4 --port_base=15000 --port_stride=12
  ./$ME --just_make --case=min_altitude_bad_config_fail
EOF
}
...
get_case_config() {
    ...
    if [ "$CASE_NAME" = "min_altitude_guard_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/min-altitude-guard-pass-shoreside.xmoos"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/min-altitude-shallow-bottom-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/min-altitude-shallow-bottom-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/min-altitude-shallow-bottom-pass-vehicle.xbhv"
        ...
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/min-altitude-unconstrained-deep-bottom-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "min_altitude_clearance_boundary_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/min-altitude-clearance-boundary-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/min-altitude-guard-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/min-altitude-clearance-boundary-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "min_altitude_zero_altitude_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/min-altitude-zero-altitude-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/min-altitude-zero-altitude-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/min-altitude-zero-altitude-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "min_altitude_bad_config_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/min-altitude-bad-config-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/min-altitude-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/min-altitude-bad-config-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "min_altitude_negative_min_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/min-altitude-negative-min-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/min-altitude-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/min-altitude-negative-min-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "min_altitude_bad_critical_bool_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/min-altitude-bad-critical-bool-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/min-altitude-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/min-altitude-bad-critical-bool-fail-vehicle.xbhv"
