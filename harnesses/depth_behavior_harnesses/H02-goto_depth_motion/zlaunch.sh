...
Examples:
  ./$ME
  ./$ME --case=goto_depth_sequence_pass
  ./$ME --jobs=4 --port_base=15000 --port_stride=12
  ./$ME --just_make --case=goto_depth_bad_sequence_fail
EOF
}
...
get_case_config() {
    ...
    if [ "$CASE_NAME" = "goto_depth_sequence_pass" ]; then
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-zero-delta-crossing-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-zero-delta-crossing-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "goto_depth_single_level_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-single-level-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-single-level-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "goto_depth_time_gate_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-time-gate-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-time-gate-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "goto_depth_capture_delta_alias_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-capture-delta-alias-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-capture-delta-alias-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "goto_depth_capture_flag_alias_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-capture-flag-alias-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-capture-flag-alias-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "goto_depth_repeat_exhaustion_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-repeat-exhaustion-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/goto-depth-repeat-exhaustion-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-repeat-exhaustion-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "goto_depth_bad_update_preserve_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-bad-update-preserve-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/goto-depth-bad-update-preserve-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-bad-update-preserve-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "goto_depth_bad_sequence_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-bad-sequence-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/goto-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-bad-sequence-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "goto_depth_negative_depth_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-negative-depth-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/goto-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-negative-depth-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "goto_depth_bad_repeat_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-bad-repeat-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/goto-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-bad-repeat-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "goto_depth_bad_arrival_delta_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-bad-arrival-delta-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/goto-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-bad-arrival-delta-fail-vehicle.xbhv"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/goto-depth-domain-missing-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/goto-depth-domain-missing-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/goto-depth-sequence-pass-vehicle.xbhv"
    else
