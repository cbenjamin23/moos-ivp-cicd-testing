...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H03-periodic_surface_motion
...
CASES=(
    periodic_surface_pass
    periodic_surface_status_vars_pass
    periodic_surface_status_variable_alias_pass
    periodic_surface_wait_window_pass
    periodic_surface_acomms_extend_pass
    periodic_surface_mark_variable_reset_pass
    periodic_surface_quadratic_ascent_pass
    periodic_surface_fullspeed_ascent_pass
    periodic_surface_timeout_reset_pass
    periodic_surface_current_speed_pass
    periodic_surface_speed_alias_pass
    periodic_surface_bad_period_fail
    periodic_surface_bad_ascent_grade_fail
    periodic_surface_bad_ascent_speed_fail
    periodic_surface_bad_zero_speed_depth_fail
    periodic_surface_bad_max_time_fail
    periodic_surface_bad_acomms_mark_fail
    periodic_surface_bad_acomms_interval_fail
    periodic_surface_bad_acomms_max_fail
    periodic_surface_bad_mark_variable_fail
    periodic_surface_bad_pending_status_var_fail
    periodic_surface_bad_atsurface_status_var_fail
    periodic_surface_missing_nav_fail
    periodic_surface_domain_missing_fail
)
...
Examples:
  ./$ME
  ./$ME --case=periodic_surface_pass
  ./$ME --jobs=4 --port_base=15000 --port_stride=12
  ./$ME --just_make --case=periodic_surface_bad_period_fail
EOF
}
...
get_case_config() {
    ...
    if [ "$CASE_NAME" = "periodic_surface_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_status_vars_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-status-vars-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-status-vars-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-status-vars-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_status_variable_alias_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-status-variable-alias-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-status-variable-alias-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-status-variable-alias-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_wait_window_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-wait-window-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-wait-window-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_acomms_extend_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-acomms-extend-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-acomms-extend-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-acomms-extend-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_mark_variable_reset_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-mark-variable-reset-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-mark-variable-reset-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-mark-variable-reset-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_quadratic_ascent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-quadratic-ascent-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-profile-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-quadratic-ascent-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_fullspeed_ascent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-fullspeed-ascent-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-profile-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-fullspeed-ascent-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_timeout_reset_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-timeout-reset-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-timeout-reset-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_current_speed_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-current-speed-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-profile-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-current-speed-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_speed_alias_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-speed-alias-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-profile-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-speed-alias-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_period_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-period-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-period-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_ascent_grade_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-ascent-grade-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-ascent-grade-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_ascent_speed_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-ascent-speed-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-ascent-speed-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_zero_speed_depth_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-zero-speed-depth-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-zero-speed-depth-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_max_time_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-max-time-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-max-time-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_acomms_mark_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-acomms-mark-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-acomms-mark-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_acomms_interval_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-acomms-interval-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-acomms-interval-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_acomms_max_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-acomms-max-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-acomms-max-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_mark_variable_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-mark-variable-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-mark-variable-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_pending_status_var_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-pending-status-var-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-pending-status-var-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_bad_atsurface_status_var_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-bad-atsurface-status-var-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-bad-atsurface-status-var-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_missing_nav_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-missing-nav-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-missing-nav-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "periodic_surface_domain_missing_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/periodic-surface-domain-missing-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/periodic-surface-domain-missing-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/periodic-surface-pass-vehicle.xbhv"
    else
...
apply_case_overlays() {
    ...
    if [ "${#vehicle_patches[@]}" -gt 0 ]; then
        for patch in "${vehicle_patches[@]}"; do [ -f "$patch" ] || return 1; done
