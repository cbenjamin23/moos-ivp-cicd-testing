...
Examples:
  ./$ME
  ./$ME --case=max_depth_guard_pass
  ./$ME --jobs=4 --port_base=15000 --port_stride=12
  ./$ME --just_make --case=max_depth_bad_config_fail
EOF
}
...
get_case_config() {
    ...
    if [ "$CASE_NAME" = "max_depth_guard_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/max-depth-guard-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-depth-guard-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "max_depth_zero_guard_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/max-depth-zero-guard-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-depth-zero-guard-pass-vehicle.xbhv"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/max-depth-no-slack-var-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-depth-no-slack-var-pass-vehicle.xbhv"
        ...
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-depth-bad-config-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "max_depth_bad_tolerance_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/max-depth-bad-tolerance-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/max-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-depth-bad-tolerance-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "max_depth_bad_basewidth_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/max-depth-bad-basewidth-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/max-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-depth-bad-basewidth-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "max_depth_bad_slack_var_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/max-depth-bad-slack-var-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/max-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-depth-bad-slack-var-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "max_depth_bad_ascent_field_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/max-depth-bad-ascent-field-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/max-depth-helm-malconfig-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-depth-bad-ascent-field-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "max_depth_missing_nav_depth_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/max-depth-missing-nav-depth-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/max-depth-missing-nav-depth-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-depth-guard-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "max_depth_domain_missing_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/max-depth-domain-missing-fail-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/max-depth-domain-missing-fail-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-depth-guard-pass-vehicle.xbhv"
    else
