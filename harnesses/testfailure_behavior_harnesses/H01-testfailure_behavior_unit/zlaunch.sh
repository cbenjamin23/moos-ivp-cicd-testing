...
MISSION_DIR="$REPO_DIR/missions/testfailure_behavior_missions/testfailure_behavior_unit"
...
BHV_STEM="$MISSION_DIR/meta_behavior.bhv"
SHORE_XFILE="$MISSION_DIR/meta_shoreside.moosx"
BHV_XFILE="$MISSION_DIR/meta_behavior.bhvx"
...
for ARGI; do
    ...
    if [ "${ARGI}" = "--help" -o "${ARGI}" = "-h" ]; then
        ...
        echo "Examples:"
        echo "  ./zlaunch.sh"
        echo "  ./zlaunch.sh --case=burn_gap_detected_pass"
        echo "  ./zlaunch.sh --jobs=4 --port_base=18000"
        exit 0
...
get_case_config() {
    ...
    if [ "$CASE_NAME" = "baseline_armed_no_trigger_pass" ]; then
        true
    elif [ "$CASE_NAME" = "burn_gap_detected_pass" ]; then
        SHORE_PATCH="$HARNESS_DIR/burn-gap-detected-pass-shoreside.xmoos"
        BHV_PATCH="$HARNESS_DIR/burn-gap-detected-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "hang_alias_gap_detected_pass" ]; then
        SHORE_PATCH="$HARNESS_DIR/hang-alias-gap-detected-pass-shoreside.xmoos"
        BHV_PATCH="$HARNESS_DIR/hang-alias-gap-detected-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "burn_default_time_gap_pass" ]; then
        SHORE_PATCH="$HARNESS_DIR/burn-default-time-gap-pass-shoreside.xmoos"
        BHV_PATCH="$HARNESS_DIR/burn-default-time-gap-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "burn_malformed_time_default_gap_pass" ]; then
        SHORE_PATCH="$HARNESS_DIR/burn-malformed-time-default-gap-pass-shoreside.xmoos"
        BHV_PATCH="$HARNESS_DIR/burn-malformed-time-default-gap-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "burn_negative_time_complete_pass" ]; then
        SHORE_PATCH="$HARNESS_DIR/burn-negative-time-complete-pass-shoreside.xmoos"
        BHV_PATCH="$HARNESS_DIR/burn-negative-time-complete-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "burn_zero_complete_pass" ]; then
        SHORE_PATCH="$HARNESS_DIR/burn-zero-complete-pass-shoreside.xmoos"
        BHV_PATCH="$HARNESS_DIR/burn-zero-complete-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "crash_on_complete_fail" ]; then
        SHORE_PATCH="$HARNESS_DIR/crash-on-complete-fail-shoreside.xmoos"
        BHV_PATCH="$HARNESS_DIR/crash-on-complete-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "unknown_type_default_crash_fail" ]; then
        SHORE_PATCH="$HARNESS_DIR/unknown-type-default-crash-fail-shoreside.xmoos"
        BHV_PATCH="$HARNESS_DIR/unknown-type-default-crash-fail-vehicle.xbhv"
    else
...
prepare_case_dir() {
    ...
    local bhv_stem="$case_dir/meta_behavior.bhv"
    local shore_xfile="$case_dir/meta_shoreside.moosx"
    local bhv_xfile="$case_dir/meta_behavior.bhvx"
...
if [ "$JOBS" -le 1 ] || [ "$CASE" != "" ]; then
    ...
    RUN_ROOT=$(mktemp -d "$HARNESS_DIR/.parallel_testfailure_unit_XXXXXX")
    ...
    while [ "$remaining_cases" != "" ]; do
        ...
        for case_tag in $wave_cases; do
            if [ -f "$CASE_ROW_DIR/${case_tag}.txt" ]; then
                ...
                line=`tail -n 1 "$RESULTS_FILE"`
                if ! case_row_passed "$line"; then
                    ALL_OK="no"
                fi
            else
                ALL_OK="no"
                echo "case=${case_tag#*_}  grade=fail  reason=missing_result_file" >> "$RESULTS_FILE"
            fi
        done
