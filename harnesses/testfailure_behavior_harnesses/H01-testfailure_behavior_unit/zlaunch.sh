...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H01-testfailure_behavior_unit
...
Options:
  ...
  --gui              Run one explicit case without headless mode
...
Examples:
  ./$ME
  ./$ME --case=burn_gap_detected_pass
  ./$ME --log=full --case=burn_gap_detected_pass
  ./$ME --jobs=4 --port_base=9000
  ./$ME --just_make --case=crash_on_complete_fail
EOF
}
...
get_case_config() {
    ...
    case "$case_name" in
        baseline_armed_no_trigger_pass)
            ;;
        burn_gap_detected_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/burn-gap-detected-pass-shoreside.xmoos"
            CASE_BHV_PATCH="$HARNESS_DIR/burn-gap-detected-pass-vehicle.xbhv"
            ;;
        hang_alias_gap_detected_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/hang-alias-gap-detected-pass-shoreside.xmoos"
            CASE_BHV_PATCH="$HARNESS_DIR/hang-alias-gap-detected-pass-vehicle.xbhv"
            ;;
        burn_default_time_gap_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/burn-default-time-gap-pass-shoreside.xmoos"
            CASE_BHV_PATCH="$HARNESS_DIR/burn-default-time-gap-pass-vehicle.xbhv"
            ;;
        burn_malformed_time_default_gap_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/burn-malformed-time-default-gap-pass-shoreside.xmoos"
            CASE_BHV_PATCH="$HARNESS_DIR/burn-malformed-time-default-gap-pass-vehicle.xbhv"
            ;;
        burn_negative_time_complete_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/burn-negative-time-complete-pass-shoreside.xmoos"
            CASE_BHV_PATCH="$HARNESS_DIR/burn-negative-time-complete-pass-vehicle.xbhv"
            ;;
        burn_zero_complete_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/burn-zero-complete-pass-shoreside.xmoos"
            CASE_BHV_PATCH="$HARNESS_DIR/burn-zero-complete-pass-vehicle.xbhv"
            ;;
        crash_on_complete_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/crash-on-complete-fail-shoreside.xmoos"
            CASE_BHV_PATCH="$HARNESS_DIR/crash-on-complete-fail-vehicle.xbhv"
            ;;
        unknown_type_default_crash_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/unknown-type-default-crash-fail-shoreside.xmoos"
            CASE_BHV_PATCH="$HARNESS_DIR/unknown-type-default-crash-fail-vehicle.xbhv"
            ;;
        *) return 1 ;;
    esac
}
...
apply_case_overlays() {
    ...
    if [ -n "$CASE_BHV_PATCH" ]; then
        [ -f "$CASE_BHV_PATCH" ] || return 1
        nspatch --stem="$workdir/meta_behavior.bhv" \
            "$CASE_BHV_PATCH" --targ="$workdir/meta_behavior.bhvx" || return 1
    fi
}
prepare_case() {
    local case_name="$1"
