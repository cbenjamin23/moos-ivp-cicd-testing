...
crash/default-crash cases, `uLoadWatch` near-breach and breach counts that cache
every `PHELMIVP_ITER_GAP` sample for burn and hang completion stalls,
`TEST_FAILURE_DONE` endflags for behavior lifecycle completion, and
`pMissionEval` result rows in `results.txt`. The four stall fixtures wait for an
ordinary iteration-gap sample before activating, proving that load monitoring
is armed before the behavior blocks the helm. Two-second cases require a gap
between 16 and 24 times the selected warp, while three-second cases require a
gap between 26 and 36 times the selected warp; at the helm's 10 Hz AppTick these
are broad, non-overlapping real-time bands. A high-rate auxiliary evaluator
also preserves the exact transient gap when scheduler timing permits, but that
diagnostic sample is not the pass/fail oracle. A short post-completion deadline
makes a missing stall produce a mission-owned failure instead of a missing
result. Expected crash cases
...
- `burn_gap_detected_pass`
  ...
  when the cached gap is between 16 and 24 times the selected warp before the
  observation deadline, `TEST_FAILURE_DONE=true`, and all watched processes
  remain present.
- `hang_alias_gap_detected_pass`
  ...
  on the same two-second gap band, the completion endflag, and preserved
  process health.
- `burn_default_time_gap_pass`
  Uses `failure_type=burn` with no time field, leaving the constructor's burn
  target at 3; passes when the cached gap is between 26 and 36 times the
  selected warp, the endflag fires, and all watched processes remain present.
- `burn_malformed_time_default_gap_pass`
  ...
  three-second gap band, completion, and process-health evidence as the
  omitted-time case.
