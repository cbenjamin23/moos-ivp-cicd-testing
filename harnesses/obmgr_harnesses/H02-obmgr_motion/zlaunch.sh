...
Examples:
  ./$ME
  ./$ME --case=baseline_center_pass
  ./$ME --jobs=2 --port_base=24000
  ./$ME --just_make --case=wide_center_fail
EOF
}
...
get_case_config() {
    ...
    case "$case_name" in
        baseline_center_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/baseline-center-pass-shoreside.xmoos"
            ;;
        offset_clear_pass)
            ...
            ;;
        tight_alert_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/tight-alert-pass-shoreside.xmoos"
            CASE_VEH_PATCH="$HARNESS_DIR/tight-alert-pass-vehicle.xmoos"
            ;;
        runtime_given_late_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/runtime-given-late-pass-shoreside.xmoos"
            CASE_VEH_PATCH="$HARNESS_DIR/runtime-given-late-pass-vehicle.xmoos"
            ;;
        point_cluster_pass)
        ...
        two_sequential_fail)
            ...
            CASE_VEH_PATCH="$HARNESS_DIR/two-sequential-pass-vehicle.xmoos"
            ;;
        wide_center_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/wide-center-fail-shoreside.xmoos"
            CASE_VEH_PATCH="$HARNESS_DIR/wide-center-fail-vehicle.xmoos"
            ;;
        avoid_disabled_fail)
        ...
        no_alert_request_fail)
            ...
            CASE_VEH_PATCH="$HARNESS_DIR/no-alert-request-fail-vehicle.xmoos"
            ;;
        *) return 1 ;;
    esac
}
