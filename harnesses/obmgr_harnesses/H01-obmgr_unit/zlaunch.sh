...
Examples:
  ./$ME
  ./$ME --log=full
  ./$ME --case=given_baseline_pass
  ./$ME --jobs=2 --port_base=20000
  ./$ME --just_make --case=points_ignore_range_absent_pass
EOF
}
...
get_case_config() {
    ...
    if [ "$case_name" = "given_baseline_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/given-baseline-pass-shoreside.xmoos"
    elif [ "$case_name" = "config_given_obstacle_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/config-given-obstacle-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/config-given-obstacle-pass-vehicle.xmoos"
    elif [ "$case_name" = "given_obstable_alias_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/given-obstable-alias-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/given-obstable-alias-pass-vehicle.xmoos"
    elif [ "$case_name" = "no_alert_request_absent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/no-alert-request-absent-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/no-alert-request-absent-pass-vehicle.xmoos"
    elif [ "$case_name" = "bad_alert_request_absent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/bad-alert-request-absent-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/bad-alert-request-absent-pass-vehicle.xmoos"
    elif [ "$case_name" = "given_far_absent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/given-far-absent-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/given-far-absent-pass-vehicle.xmoos"
    elif [ "$case_name" = "given_general_alert_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/given-general-alert-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/given-general-alert-pass-vehicle.xmoos"
    elif [ "$case_name" = "general_alert_default_name_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/general-alert-default-name-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/general-alert-default-name-pass-vehicle.xmoos"
    elif [ "$case_name" = "given_duration_resolve_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/given-duration-resolve-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/given-duration-resolve-pass-vehicle.xmoos"
    elif [ "$case_name" = "given_max_duration_reject_absent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/given-max-duration-reject-absent-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/given-max-duration-reject-absent-pass-vehicle.xmoos"
    elif [ "$case_name" = "given_max_duration_missing_absent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/given-max-duration-missing-absent-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/given-max-duration-missing-absent-pass-vehicle.xmoos"
    elif [ "$case_name" = "invalid_given_nonconvex_absent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/invalid-given-nonconvex-absent-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/invalid-given-nonconvex-absent-pass-vehicle.xmoos"
    elif [ "$case_name" = "post_dist_always_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/post-dist-always-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/post-dist-always-pass-vehicle.xmoos"
    elif [ "$case_name" = "post_dist_false_absent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/post-dist-false-absent-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/post-dist-false-absent-pass-vehicle.xmoos"
    elif [ "$case_name" = "points_cluster_dist_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/points-cluster-dist-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/points-cluster-dist-pass-vehicle.xmoos"
    elif [ "$case_name" = "custom_point_var_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/custom-point-var-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/custom-point-var-pass-vehicle.xmoos"
    elif [ "$case_name" = "invalid_point_missing_key_absent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/invalid-point-missing-key-absent-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/invalid-point-missing-key-absent-pass-vehicle.xmoos"
    elif [ "$case_name" = "max_pts_per_cluster_trim_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/max-pts-per-cluster-trim-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/max-pts-per-cluster-trim-pass-vehicle.xmoos"
    elif [ "$case_name" = "points_ignore_range_absent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/points-ignore-range-absent-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/points-ignore-range-absent-pass-vehicle.xmoos"
    elif [ "$case_name" = "points_age_resolve_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/points-age-resolve-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/points-age-resolve-pass-vehicle.xmoos"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/placeholder-hull-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/placeholder-hull-pass-vehicle.xmoos"
    elif [ "$case_name" = "post_view_polys_false_absent_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/post-view-polys-false-absent-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/post-view-polys-false-absent-pass-vehicle.xmoos"
    elif [ "$case_name" = "new_obs_flag_macros_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/new-obs-flag-macros-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/new-obs-flag-macros-pass-vehicle.xmoos"
    elif [ "$case_name" = "point_vsource_alert_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/point-vsource-alert-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/point-vsource-alert-pass-vehicle.xmoos"
    elif [ "$case_name" = "given_after_points_same_key_reject_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/given-after-points-same-key-reject-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/given-after-points-same-key-reject-pass-vehicle.xmoos"
    elif [ "$case_name" = "disable_obstacle_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/disable-obstacle-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/disable-obstacle-pass-vehicle.xmoos"
    elif [ "$case_name" = "disable_vsource_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/disable-vsource-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/disable-vsource-pass-vehicle.xmoos"
    elif [ "$case_name" = "enable_obstacle_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/enable-obstacle-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/enable-obstacle-pass-vehicle.xmoos"
    elif [ "$case_name" = "expunge_obstacle_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/expunge-obstacle-pass-shoreside.xmoos"
        CASE_VEH_PATCH="$HARNESS_DIR/expunge-obstacle-pass-vehicle.xmoos"
    else
        return 1
    fi
}
