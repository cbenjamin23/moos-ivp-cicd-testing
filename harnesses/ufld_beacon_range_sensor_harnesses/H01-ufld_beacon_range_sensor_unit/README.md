...
- `brs_ground_truth_false_no_gt_pass` Configures zero-percent uniform noise, `report_vars=both`, and `ground_truth=false`; passes when both ordinary report forms identify the beacon at range `50` while neither `BRS_RANGE_REPORT_GT` nor requester-specific `BRS_RANGE_REPORT_GT_ALPHA` is published during the six-second window.
