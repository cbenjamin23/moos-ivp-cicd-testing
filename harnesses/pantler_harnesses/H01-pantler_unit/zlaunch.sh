...
MISSION_DIR="$REPO_DIR/missions/pantler_missions/pantler_unit"
...
Examples:
  ./$ME
  ./$ME --case=pantler_multi_alias_launch_pass
  ./$ME --jobs=2 --port_base=9000
  ./$ME --just_make --case=pantler_filter_launch_pass
EOF
}
...
get_case_config() {
    ...
    case "$case_name" in
        pantler_baseline_launch_pass)
            ;;
        pantler_alias_launch_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/pantler-alias-launch-pass-shoreside.xmoos"
            ;;
        pantler_multi_alias_launch_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/pantler-multi-alias-launch-pass-shoreside.xmoos"
            ;;
        pantler_filter_launch_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/pantler-filter-launch-pass-shoreside.xmoos"
            ;;
        pantler_system_path_launch_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/pantler-system-path-launch-pass-shoreside.xmoos"
            ;;
        pantler_extra_params_launch_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/pantler-extra-params-launch-pass-shoreside.xmoos"
            ;;
        *) return 1 ;;
    esac
}
