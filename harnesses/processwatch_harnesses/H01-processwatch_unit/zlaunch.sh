...
MISSION_DIR="$REPO_DIR/missions/processwatch_missions/processwatch_unit"
...
Examples:
  ./$ME
  ./$ME --case=processwatch_all_present_pass
  ./$ME --log=full --case=processwatch_all_present_pass
  ./$ME --jobs=3 --port_base=11600
  ./$ME --just_make --case=processwatch_missing_awol_fail
EOF
}
...
get_case_config() {
    ...
    case "$case_name" in
        processwatch_all_present_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/processwatch-all-present-pass-shoreside.xmoos"
            ;;
        processwatch_custom_present_post_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/processwatch-custom-present-post-pass-shoreside.xmoos"
            ;;
        processwatch_multi_present_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/processwatch-multi-present-pass-shoreside.xmoos"
            ;;
        processwatch_post_mapping_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/processwatch-post-mapping-pass-shoreside.xmoos"
            ;;
        processwatch_full_summary_mapping_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/processwatch-full-summary-mapping-pass-shoreside.xmoos"
            ;;
        processwatch_event_mapping_present_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/processwatch-event-mapping-present-pass-shoreside.xmoos"
            ;;
        processwatch_missing_awol_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/processwatch-missing-awol-fail-shoreside.xmoos"
            ;;
        *) return 1 ;;
    esac
}
