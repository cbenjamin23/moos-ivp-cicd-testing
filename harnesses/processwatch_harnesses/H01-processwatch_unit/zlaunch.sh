...
for ARGI; do
    ...
    if [ "${ARGI}" = "--help" -o "${ARGI}" = "-h" ]; then
        ...
        echo "Examples:"
        echo "  ./zlaunch.sh"
        echo "  ./zlaunch.sh --case=processwatch_missing_awol_fail"
        echo "  ./zlaunch.sh --jobs=4 --port_base=11600"
        exit 0
...
if [ "$JOBS" -le 1 ] || [ "$CASE" != "" ]; then
    ...
    RUN_ROOT=$(mktemp -d "$HARNESS_DIR/.parallel_processwatch_unit_XXXXXX")
