...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H01-memoryturnlimit_behavior_motion
...
Examples:
  ./$ME
  ./$ME --case=baseline_memory_pass
  ./$ME --jobs=4 --port_base=15000 --port_stride=12
  ./$ME --just_make --case=bad_memory_time_fail
EOF
}
...
get_case_config() {
    ...
    case "$CASE_NAME" in
        baseline_memory_pass)
            ;;
        tight_window_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-tight-window-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/tight-window-pass-vehicle.xbhv"
            ;;
        relaxed_window_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-relaxed-window-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/relaxed-window-pass-vehicle.xbhv"
            ;;
        short_memory_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-short-memory-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/short-memory-pass-vehicle.xbhv"
            ;;
        long_memory_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-long-memory-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/long-memory-pass-vehicle.xbhv"
            ;;
        zero_memory_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-zero-memory-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/zero-memory-pass-vehicle.xbhv"
            ;;
        very_slow_speed_floor_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-very-slow-speed-floor-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/very-slow-speed-floor-pass-vehicle.xbhv"
            ;;
        weak_priority_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-weak-priority-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/weak-priority-pass-vehicle.xbhv"
            ;;
        slow_speed_scaled_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-slow-speed-scaled-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/slow-speed-scaled-pass-vehicle.xbhv"
            ;;
        fast_speed_unscaled_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-fast-speed-unscaled-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/fast-speed-unscaled-pass-vehicle.xbhv"
            ;;
        moderate_turn_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-moderate-turn-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/moderate-turn-pass-vehicle.xbhv"
            ;;
        starboard_turn_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-starboard-turn-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/starboard-turn-pass-vehicle.xbhv"
            ;;
        max_turn_range_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-max-turn-range-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/max-turn-range-pass-vehicle.xbhv"
            ;;
        zero_turn_range_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-zero-turn-range-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/zero-turn-range-pass-vehicle.xbhv"
            ;;
        runtime_widen_range_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-runtime-widen-range-pass-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/runtime-widen-range-pass-vehicle.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/runtime-widen-range-pass-vehicle.xbhv"
            ;;
        runtime_tighten_range_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-runtime-tighten-range-pass-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/runtime-tighten-range-pass-vehicle.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/runtime-tighten-range-pass-vehicle.xbhv"
            ;;
        bad_memory_time_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-helm-malconfig-fail-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/helm-malconfig-fail-vehicle.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-memory-time-fail-vehicle.xbhv"
            ;;
        bad_turn_range_high_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-helm-malconfig-fail-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/helm-malconfig-fail-vehicle.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-turn-range-high-fail-vehicle.xbhv"
            ;;
        bad_turn_range_text_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-helm-malconfig-fail-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/helm-malconfig-fail-vehicle.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-turn-range-text-fail-vehicle.xbhv"
            ;;
        missing_memory_time_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-bhv-warning-fail-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/missing-memory-time-fail-vehicle.xbhv"
            ;;
        missing_turn_range_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/eval-bhv-warning-fail-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/missing-turn-range-fail-vehicle.xbhv"
            ;;
        *)
