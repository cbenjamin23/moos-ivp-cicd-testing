...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H01-timer_behavior_motion
...
Examples:
  ./$ME
  ./$ME --log=full
  ./$ME --case=custom_status_vars_pass
...
get_case_config() {
    ...
    case "$CASE_NAME" in
        ...
        custom_status_vars_pass)
            ...
            ;;
        default_suffix_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/default-suffix-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/default-suffix-pass-vehicle.xbhv"
            ;;
        custom_no_suffix_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/custom-no-suffix-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/custom-no-suffix-pass-vehicle.xbhv"
            ;;
        default_no_suffix_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/default-no-suffix-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/default-no-suffix-pass-vehicle.xbhv"
            ;;
        active_at_start_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/active-at-start-pass-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/active-at-start-pass-vehicle.xmoos"
            ;;
        delayed_activation_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/delayed-activation-pass-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/delayed-activation-pass-vehicle.xmoos"
            ;;
        pause_resume_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/pause-resume-pass-shoreside.xmoos"
        ...
        repeated_status_param_pass)
            ...
            ;;
        post_mapping_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/post-mapping-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/post-mapping-pass-vehicle.xbhv"
            ;;
        duration_complete_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/duration-complete-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/duration-complete-pass-vehicle.xbhv"
            ;;
        runtime_update_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/runtime-update-pass-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/runtime-update-pass-vehicle.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/runtime-update-pass-vehicle.xbhv"
            ;;
        never_active_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/never-active-fail-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/never-active-fail-vehicle.xmoos"
            ;;
        bad_idle_var_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/no-status-output-fail-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-idle-var-fail-vehicle.xbhv"
            ;;
        bad_running_var_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/no-status-output-fail-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-running-var-fail-vehicle.xbhv"
            ;;
        bad_suffix_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/no-status-output-fail-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-suffix-fail-vehicle.xbhv"
            ;;
        post_mapping_silent_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/post-mapping-silent-fail-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/post-mapping-silent-fail-vehicle.xbhv"
            ;;
        unknown_param_fail)
            CASE_SHORE_PATCH="$HARNESS_DIR/no-status-output-fail-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/unknown-param-fail-vehicle.xbhv"
            ;;
        *)
