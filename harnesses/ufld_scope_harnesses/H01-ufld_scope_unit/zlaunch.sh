...
COMMON_PATCH="$HARNESS_DIR/scope-common-shoreside.xmoos"
...
get_case_patch() {
    local case_name="$1"
    case "$case_name" in
        appcast_table_pass) CASE_PATCH="$HARNESS_DIR/appcast-table-pass-shoreside.xmoos" ;;
        alias_headers_pass) CASE_PATCH="$HARNESS_DIR/alias-headers-pass-shoreside.xmoos" ;;
        update_replaces_same_key_pass) CASE_PATCH="$HARNESS_DIR/update-replaces-same-key-pass-shoreside.xmoos" ;;
        multi_vehicle_rows_pass) CASE_PATCH="$HARNESS_DIR/multi-vehicle-rows-pass-shoreside.xmoos" ;;
        invalid_scope_ignored_pass) CASE_PATCH="$HARNESS_DIR/invalid-scope-ignored-pass-shoreside.xmoos" ;;
        missing_field_blank_cell_pass) CASE_PATCH="$HARNESS_DIR/missing-field-blank-cell-pass-shoreside.xmoos" ;;
        missing_key_blank_row_pass) CASE_PATCH="$HARNESS_DIR/missing-key-blank-row-pass-shoreside.xmoos" ;;
        later_missing_field_replaces_value_pass) CASE_PATCH="$HARNESS_DIR/later-missing-field-replaces-value-pass-shoreside.xmoos" ;;
        *) return 1 ;;
    esac
}
...
prepare_case() {
    ...
    if [ "$LOG_MODE" = minimal ]; then
        (
            cd "$workdir" || exit 1
            ./prepare_logging_mode.sh "$LOG_MODE" appcast meta_shoreside.moosx
        ) || return 1
    fi
}

latest_scope_appcast() {
    local workdir="$1"
    local alog candidate latest=""
    while IFS= read -r -d '' alog; do
        candidate=$(rg '^[[:space:]]*-?[0-9.]+[[:space:]]+APPCAST[[:space:]]+uFldScope[[:space:]]+proc=uFldScope' "$alog" 2>/dev/null | tail -n 1 || true)
