...
- `refresh_interval_repost_pass` Sets `refresh_interval=0.6` and triggers once at time 1; passes when both truth-channel counts are between 4 and 6 at time 2.5 and between 8 and 10 at time 6, with `ufos_a` present on both channels.
- `refresh_interval_post_points_no_given_pass` Uses the same 0.6-second interval with `post_points=true` and `rate_points=0`; passes when the known-truth count is between 4 and 6 at time 2.5 and between 8 and 10 at time 6 while `GIVEN_OBSTACLE` remains absent.
...
- `reset_request_inside_blocked_pass` Refreshes the field, places Alpha at `(20,0)` inside the field region, and posts `UFOS_RESET=true` with `reset_range=10`; passes when the reset input is observed, the PMV trigger and Alpha's first-report query produce exactly two known and two given posts, and no clear or later republish occurs.
...
- `reset_range_boundary_blocked_pass` Places Alpha at `(70,0)`, exactly 10 meters outside the field-region boundary, and posts reset with `reset_range=10`; passes when the PMV trigger and Alpha's first-report query produce exactly two known and two given posts, and the reset produces no clear or later republish.
...
- `reset_interval_wait_blocked_pass` Refreshes the field, moves Alpha from inside to outside after 0.9 seconds, but sets `reset_interval=100`; passes when the PMV trigger and Alpha's first-report query produce exactly two known and two given posts and neither an automatic clear nor a later republish occurs.
