        ...
        if [ "$KEEP_WORKDIRS" != yes ] && [ "$root_stopped" = yes ] &&
           [ "$CLEANUP_FAILED" = no ]; then
            if ! rm -rf -- "$RUN_ROOT" || [ -e "$RUN_ROOT" ]; then
                echo "$ME: unable to remove run root: $RUN_ROOT" >&2
                CLEANUP_FAILED=yes
            fi
        fi
    fi
...
get_case_config() {
    ...
    case "$case_name" in
        fixed_field_publish_pass) ;;
        multi_field_labels_pass) CASE_SHORE_PATCH="$HARNESS_DIR/multi-field-labels-pass-shoreside.xmoos" ;;
        duration_fixed_pass) CASE_SHORE_PATCH="$HARNESS_DIR/duration-fixed-pass-shoreside.xmoos" ;;
        duration_min_only_pass) CASE_SHORE_PATCH="$HARNESS_DIR/duration-min-only-pass-shoreside.xmoos" ;;
        duration_min_max_swap_pass) CASE_SHORE_PATCH="$HARNESS_DIR/duration-min-max-swap-pass-shoreside.xmoos" ;;
        duration_disabled_zero_pass) CASE_SHORE_PATCH="$HARNESS_DIR/duration-disabled-zero-pass-shoreside.xmoos" ;;
        pmv_connect_repeated_repost_pass) CASE_SHORE_PATCH="$HARNESS_DIR/pmv-connect-repeated-repost-pass-shoreside.xmoos" ;;
        pmv_connect_absent_no_publish_pass) CASE_SHORE_PATCH="$HARNESS_DIR/pmv-connect-absent-no-publish-pass-shoreside.xmoos" ;;
        vehicle_connect_no_refresh_pass) CASE_SHORE_PATCH="$HARNESS_DIR/vehicle-connect-no-refresh-pass-shoreside.xmoos" ;;
        post_visuals_false_pass) CASE_SHORE_PATCH="$HARNESS_DIR/post-visuals-false-pass-shoreside.xmoos" ;;
        draw_region_false_pass) CASE_SHORE_PATCH="$HARNESS_DIR/draw-region-false-pass-shoreside.xmoos" ;;
        post_points_inside_pass) CASE_SHORE_PATCH="$HARNESS_DIR/post-points-inside-pass-shoreside.xmoos" ;;
        post_points_multi_field_pass) CASE_SHORE_PATCH="$HARNESS_DIR/post-points-multi-field-pass-shoreside.xmoos" ;;
        post_points_multi_vehicle_pass) CASE_SHORE_PATCH="$HARNESS_DIR/post-points-multi-vehicle-pass-shoreside.xmoos" ;;
        invalid_node_report_no_refresh_pass) CASE_SHORE_PATCH="$HARNESS_DIR/invalid-node-report-no-refresh-pass-shoreside.xmoos" ;;
        invalid_node_report_no_points_pass) CASE_SHORE_PATCH="$HARNESS_DIR/invalid-node-report-no-points-pass-shoreside.xmoos" ;;
        node_report_mixed_case_vehicle_pass) CASE_SHORE_PATCH="$HARNESS_DIR/node-report-mixed-case-vehicle-pass-shoreside.xmoos" ;;
        node_report_color_point_pass) CASE_SHORE_PATCH="$HARNESS_DIR/node-report-color-point-pass-shoreside.xmoos" ;;
        sensor_range_outside_pass) CASE_SHORE_PATCH="$HARNESS_DIR/sensor-range-outside-pass-shoreside.xmoos" ;;
        sensor_range_edge_pass) CASE_SHORE_PATCH="$HARNESS_DIR/sensor-range-edge-pass-shoreside.xmoos" ;;
        sensor_range_zero_suppresses_pass) CASE_SHORE_PATCH="$HARNESS_DIR/sensor-range-zero-suppresses-pass-shoreside.xmoos" ;;
        point_size_bigger_pass) CASE_SHORE_PATCH="$HARNESS_DIR/point-size-bigger-pass-shoreside.xmoos" ;;
        point_size_numeric_pass) CASE_SHORE_PATCH="$HARNESS_DIR/point-size-numeric-pass-shoreside.xmoos" ;;
        point_size_smaller_pass) CASE_SHORE_PATCH="$HARNESS_DIR/point-size-smaller-pass-shoreside.xmoos" ;;
        point_size_sequence_pass) CASE_SHORE_PATCH="$HARNESS_DIR/point-size-sequence-pass-shoreside.xmoos" ;;
        point_size_invalid_ignored_pass) CASE_SHORE_PATCH="$HARNESS_DIR/point-size-invalid-ignored-pass-shoreside.xmoos" ;;
        point_size_floor_ignored_pass) CASE_SHORE_PATCH="$HARNESS_DIR/point-size-floor-ignored-pass-shoreside.xmoos" ;;
        point_size_uppercase_pass) CASE_SHORE_PATCH="$HARNESS_DIR/point-size-uppercase-pass-shoreside.xmoos" ;;
        point_size_fraction_pass) CASE_SHORE_PATCH="$HARNESS_DIR/point-size-fraction-pass-shoreside.xmoos" ;;
        point_size_zero_ignored_pass) CASE_SHORE_PATCH="$HARNESS_DIR/point-size-zero-ignored-pass-shoreside.xmoos" ;;
        rate_points_three_pass) CASE_SHORE_PATCH="$HARNESS_DIR/rate-points-three-pass-shoreside.xmoos" ;;
        rate_points_zero_pass) CASE_SHORE_PATCH="$HARNESS_DIR/rate-points-zero-pass-shoreside.xmoos" ;;
        post_points_no_visuals_pass) CASE_SHORE_PATCH="$HARNESS_DIR/post-points-no-visuals-pass-shoreside.xmoos" ;;
        node_report_refresh_pass) CASE_SHORE_PATCH="$HARNESS_DIR/node-report-refresh-pass-shoreside.xmoos" ;;
        refresh_interval_repost_pass) CASE_SHORE_PATCH="$HARNESS_DIR/refresh-interval-repost-pass-shoreside.xmoos" ;;
        refresh_interval_post_points_no_given_pass) CASE_SHORE_PATCH="$HARNESS_DIR/refresh-interval-post-points-no-given-pass-shoreside.xmoos" ;;
        reset_request_reposts_pass) CASE_SHORE_PATCH="$HARNESS_DIR/reset-request-reposts-pass-shoreside.xmoos" ;;
        reset_request_inside_blocked_pass) CASE_SHORE_PATCH="$HARNESS_DIR/reset-request-inside-blocked-pass-shoreside.xmoos" ;;
        reset_reuse_ids_false_pass) CASE_SHORE_PATCH="$HARNESS_DIR/reset-reuse-ids-false-pass-shoreside.xmoos" ;;
        reset_post_points_no_given_pass) CASE_SHORE_PATCH="$HARNESS_DIR/reset-post-points-no-given-pass-shoreside.xmoos" ;;
        reset_post_visuals_false_pass) CASE_SHORE_PATCH="$HARNESS_DIR/reset-post-visuals-false-pass-shoreside.xmoos" ;;
        reset_range_boundary_blocked_pass) CASE_SHORE_PATCH="$HARNESS_DIR/reset-range-boundary-blocked-pass-shoreside.xmoos" ;;
        reset_interval_exit_pass) CASE_SHORE_PATCH="$HARNESS_DIR/reset-interval-exit-pass-shoreside.xmoos" ;;
        reset_interval_wait_blocked_pass) CASE_SHORE_PATCH="$HARNESS_DIR/reset-interval-wait-blocked-pass-shoreside.xmoos" ;;
        sensor_range_broad_far_pass) CASE_SHORE_PATCH="$HARNESS_DIR/sensor-range-broad-far-pass-shoreside.xmoos" ;;
        obstacle_visual_style_pass) CASE_SHORE_PATCH="$HARNESS_DIR/obstacle-visual-style-pass-shoreside.xmoos" ;;
        region_visual_style_pass) CASE_SHORE_PATCH="$HARNESS_DIR/region-visual-style-pass-shoreside.xmoos" ;;
        poly_visual_zero_sizes_pass) CASE_SHORE_PATCH="$HARNESS_DIR/poly-visual-zero-sizes-pass-shoreside.xmoos" ;;
        missing_obstacle_file_absent_pass) CASE_SHORE_PATCH="$HARNESS_DIR/missing-obstacle-file-absent-pass-shoreside.xmoos" ;;
        *) return 1 ;;
    esac
}
...
start_case() {
    ...
    if ! mkdir -p "$case_dir"; then
        echo "$ME: unable to create case directory: $case_dir" >&2
        return 1
    fi
