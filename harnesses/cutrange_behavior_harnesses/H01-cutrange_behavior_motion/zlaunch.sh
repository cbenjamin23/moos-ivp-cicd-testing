...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H01-cutrange_behavior_motion
...
Examples:
  ./$ME
  ./$ME --log=full
  ./$ME --case=static_cutrange_pass
  ./$ME --jobs=4 --port_base=15000 --port_stride=13
  ./$ME --just_make --case=bad_legacy_util_param_fail
EOF
}
...
get_case_config() {
    ...
    if [ "$CASE_NAME" = "startup_no_warning_pass" ]; then
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-wide-closure-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/target-crossing-pass-vehicle.xbhv"
        CASE_EXTRA_ARGS="--vpos2=x=20,y=-140,heading=0"
    elif [ "$CASE_NAME" = "headon_cutrange_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-headon-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/target-headon-pass-vehicle.xbhv"
        CASE_EXTRA_ARGS="--vpos2=x=90,y=-80,heading=270"
    elif [ "$CASE_NAME" = "angled_entry_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-wide-closure-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/target-angled-pass-vehicle.xbhv"
        CASE_EXTRA_ARGS="--vpos1=x=-95,y=-125,heading=45 --vpos2=x=10,y=-80,heading=80"
    elif [ "$CASE_NAME" = "right_turn_target_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-right-turn-post-turn-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/target-right-turn-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "s_turn_target_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-s-turn-post-turn-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/target-s-turn-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "slow_target_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-slow-target-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/target-slow-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "fast_target_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-fast-target-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/target-fast-pass-vehicle.xbhv"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-zero-relevance-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/cutrange-pwt-equal-zero-pass-vehicle.xbhv"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-late-closure-pass-shoreside.xmoos"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-quick-fail-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-max-patience-fail-vehicle.xbhv"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-quick-fail-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-legacy-util-param-fail-vehicle.xbhv"
    else
