...
run_case_isolated() {
    ...
    echo "case=$case_name  case_result=$status  expected_rc=$EXPECTED_RC  actual_rc=$query_rc  readiness=$readiness  checkvars=${#CHECKVAR_TOKENS[@]}  absent=${#ABSENT_TOKENS[@]}  $line" > "$case_result_file"
