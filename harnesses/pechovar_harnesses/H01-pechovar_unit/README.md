...
- `default_repeated_source_posts_each_pass`: Leaves `echo_latest_only` false and posts `first` then `second` to one source, testing release of every queued update; passes when the final value is `second` and the logs contain exactly two `ECHO_OUT_LATEST` publications.
- `latest_only_repeated_source_pass`: Sets `echo_latest_only=true` and posts `first` then `second` before release, testing retention of the most recent source value; passes when pMissionEval observes final `ECHO_OUT_LATEST=second`, without checking the target publication count.
...
- `flip_unmapped_component_omitted_pass`: Maps only `x` from `x=8,z=99`, testing omission of an unmapped source component; passes when `xcenter=8` appears and the combined token `xcenter=8,z=99` does not.
...
- `flip_empty_value_pass`: Maps source component `x` from the payload `x=`, testing preservation of an empty component value; passes when the logs contain `xcenter=`.
...
- `invalid_flip_alias_no_output_pass`: Configures `FLIP_SRC` as both source and destination with an `x -> xcenter` mapping, testing source/destination alias rejection; passes when input `x=17` produces no `xcenter=17` payload.
- `shared_source_dual_flip_pass`: Gives two named EFlippers the same `FLIP_SRC=x=31,y=41`, mapping `x` to `FLIP_OUT` and `y` to `ECHO_OUT_STR`; passes when both `xcenter=31` and `ypos=41` appear.
- `dual_flip_independent_pass`: Gives two named EFlippers separate sources and destinations, posting `x=44` to one and `word=bravo` to the other; passes when the logs contain both `xcenter=44` and `word_out=bravo`.
