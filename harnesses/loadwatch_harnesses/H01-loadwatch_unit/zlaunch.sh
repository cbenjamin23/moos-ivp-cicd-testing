...
MISSION_DIR="$REPO_DIR/missions/loadwatch_missions/loadwatch_unit"
...
Examples:
  ./$ME
  ./$ME --case=loadwatch_no_breach_pass
  ./$ME --log=full --case=loadwatch_no_breach_pass
  ./$ME --jobs=3 --port_base=11000
  ./$ME --just_make --case=loadwatch_breach_pass
EOF
}
...
get_case_config() {
    ...
    case "$case_name" in
        loadwatch_no_breach_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-no-breach-pass-shoreside.xmoos"
            ;;
        loadwatch_near_breach_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-near-breach-pass-shoreside.xmoos"
            ;;
        loadwatch_near_boundary_no_breach_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-near-boundary-no-breach-pass-shoreside.xmoos"
            ;;
        loadwatch_breach_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-breach-pass-shoreside.xmoos"
            ;;
        loadwatch_trigger_suppresses_first_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-trigger-suppresses-first-pass-shoreside.xmoos"
            ;;
        loadwatch_trigger_second_breaches_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-trigger-second-breaches-pass-shoreside.xmoos"
            ;;
        loadwatch_lowercase_any_no_breach_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-lowercase-any-no-breach-pass-shoreside.xmoos"
            ;;
        loadwatch_specific_app_breach_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-specific-app-breach-pass-shoreside.xmoos"
            ;;
        loadwatch_specific_app_ignores_other_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-specific-app-ignores-other-pass-shoreside.xmoos"
            ;;
        loadwatch_hard_boundary_no_breach_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-hard-boundary-no-breach-pass-shoreside.xmoos"
            ;;
        loadwatch_low_threshold_ignored_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-low-threshold-ignored-pass-shoreside.xmoos"
            ;;
        loadwatch_near_thresh_clamped_high_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/loadwatch-near-thresh-clamped-high-pass-shoreside.xmoos"
            ;;
        *) return 1 ;;
    esac
}
