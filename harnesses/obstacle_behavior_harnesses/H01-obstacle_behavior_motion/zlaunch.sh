...
run_case() {
    ...
    if [ "$PORT_BASE_SET" = "yes" ]; then
        ...
        if ! check_case_ports "$case_name" "$shore_mport" "$veh_mport" "$shore_pshare" "$veh_pshare"; then
            echo "case=$case_name  grade=fail  reason=port_busy" >> "$RESULTS_FILE"
            ALL_OK="no"
            cd "$HARNESS_DIR"
            return 0
        fi
...
run_case_isolated() {
    ...
    result_file="$RESULT_DIR/${case_tag}.txt"

    get_case_config "$case_name" || {
        echo "case=$case_name  grade=fail  reason=case_config_error" > "$result_file"
        return 1
    }
    ...
    if ! check_case_ports "$case_name" "$shore_mport" "$veh_mport" "$shore_pshare" "$veh_pshare"; then
        echo "case=$case_name  grade=fail  reason=port_busy" > "$result_file"
        return 1
    fi
    ...
    if [ "$JUST_MAKE" = "yes" ]; then
        if [ "$launch_rc" = 0 ]; then
            echo "case=$case_name  grade=pass  reason=just_make" > "$result_file"
            return 0
        fi
