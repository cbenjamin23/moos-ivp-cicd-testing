...
get_case_patch() {
    local case_name="$1"
    case "$case_name" in
        collision_flag_pass) CASE_PATCH="$HARNESS_DIR/collision-flag-pass-shoreside.xmoos" ;;
        near_miss_flag_pass) CASE_PATCH="$HARNESS_DIR/near-miss-flag-pass-shoreside.xmoos" ;;
        encounter_only_flag_pass) CASE_PATCH="$HARNESS_DIR/encounter-only-flag-pass-shoreside.xmoos" ;;
        global_min_distance_pass) CASE_PATCH="$HARNESS_DIR/global-min-distance-pass-shoreside.xmoos" ;;
        invalid_obstacle_absent_pass) CASE_PATCH="$HARNESS_DIR/invalid-obstacle-absent-pass-shoreside.xmoos" ;;
        obstacle_clear_blocks_later_pass) CASE_PATCH="$HARNESS_DIR/obstacle-clear-blocks-later-pass-shoreside.xmoos" ;;
        upper_macro_flag_pass) CASE_PATCH="$HARNESS_DIR/upper-macro-flag-pass-shoreside.xmoos" ;;
        fresh_clear_keeps_obstacle_pass) CASE_PATCH="$HARNESS_DIR/fresh-clear-keeps-obstacle-pass-shoreside.xmoos" ;;
        encounter_double_flag_pass) CASE_PATCH="$HARNESS_DIR/encounter-double-flag-pass-shoreside.xmoos" ;;
        near_miss_upper_macro_flag_pass) CASE_PATCH="$HARNESS_DIR/near-miss-upper-macro-flag-pass-shoreside.xmoos" ;;
        collision_boundary_counts_near_pass) CASE_PATCH="$HARNESS_DIR/collision-boundary-counts-near-pass-shoreside.xmoos" ;;
        near_boundary_counts_encounter_pass) CASE_PATCH="$HARNESS_DIR/near-boundary-counts-encounter-pass-shoreside.xmoos" ;;
        named_clear_only_removes_target_pass) CASE_PATCH="$HARNESS_DIR/named-clear-only-removes-target-pass-shoreside.xmoos" ;;
        range_normalization_collision_pass) CASE_PATCH="$HARNESS_DIR/range-normalization-collision-pass-shoreside.xmoos" ;;
        *) return 1 ;;
    esac
}
