...
- `runtime_stealth_block_pass` Starts with `comms_range=100`, delivers Abe's `UFC_BEFORE` message, posts `UNC_STEALTH=vname=abe,stealth=0.4`, and sends `UFC_AFTER`; passes when Ben receives exactly one message—the exact before-update payload—while both reverse acknowledgements arrive.
...
- `shared_node_reports_numeric_pass` Configures `shared_node_reports=0.1`; passes when the baseline deliveries occur and at least one `NODE_REPORT_UNC` is published through the numeric configuration path.
