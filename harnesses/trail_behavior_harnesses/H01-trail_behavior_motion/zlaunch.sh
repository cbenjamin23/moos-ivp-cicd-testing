...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H01-trail_behavior_motion
...
get_case_config() {
    ...
    if [ "$CASE_NAME" = "static_trail_pass" ]; then
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/runtime-bad-update-recover-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/runtime-bad-update-recover-pass-vehicle.xmoos"
    elif [ "$CASE_NAME" = "idle_post_distance_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/idle-post-distance-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/idle-post-distance-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/idle-post-distance-pass-vehicle.xbhv"
        ...
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/auto-alert-request-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "missing_contact_warn_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/missing-contact-warn-pass-shoreside.xmoos"
