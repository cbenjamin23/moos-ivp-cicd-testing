...
COMMON_PATCH="$HARNESS_DIR/pathcheck-common-shoreside.xmoos"
...
get_case_patch() {
    local case_name="$1"
    case "$case_name" in
        odometry_baseline_pass) CASE_PATCH="$HARNESS_DIR/odometry-baseline-pass-shoreside.xmoos" ;;
        two_reports_speed_only_pass) CASE_PATCH="$HARNESS_DIR/two-reports-speed-only-pass-shoreside.xmoos" ;;
        trip_reset_pass) CASE_PATCH="$HARNESS_DIR/trip-reset-pass-shoreside.xmoos" ;;
        node_report_local_pass) CASE_PATCH="$HARNESS_DIR/node-report-local-pass-shoreside.xmoos" ;;
        mixed_report_sources_pass) CASE_PATCH="$HARNESS_DIR/mixed-report-sources-pass-shoreside.xmoos" ;;
        multi_node_independent_pass) CASE_PATCH="$HARNESS_DIR/multi-node-independent-pass-shoreside.xmoos" ;;
        variable_speed_average_pass) CASE_PATCH="$HARNESS_DIR/variable-speed-average-pass-shoreside.xmoos" ;;
        history_length_three_average_pass) CASE_PATCH="$HARNESS_DIR/history-length-three-average-pass-shoreside.xmoos" ;;
        invalid_history_length_default_pass) CASE_PATCH="$HARNESS_DIR/invalid-history-length-default-pass-shoreside.xmoos" ;;
        reverse_leg_accumulates_pass) CASE_PATCH="$HARNESS_DIR/reverse-leg-accumulates-pass-shoreside.xmoos" ;;
        history_length_two_speed_pass) CASE_PATCH="$HARNESS_DIR/history-length-two-speed-pass-shoreside.xmoos" ;;
        invalid_report_rejected_pass) CASE_PATCH="$HARNESS_DIR/invalid-report-rejected-pass-shoreside.xmoos" ;;
        stationary_zero_distance_pass) CASE_PATCH="$HARNESS_DIR/stationary-zero-distance-pass-shoreside.xmoos" ;;
        diagonal_chain_distance_pass) CASE_PATCH="$HARNESS_DIR/diagonal-chain-distance-pass-shoreside.xmoos" ;;
        trip_reset_unknown_ignored_pass) CASE_PATCH="$HARNESS_DIR/trip-reset-unknown-ignored-pass-shoreside.xmoos" ;;
        trip_reset_twice_pass) CASE_PATCH="$HARNESS_DIR/trip-reset-twice-pass-shoreside.xmoos" ;;
        bad_then_good_recovery_pass) CASE_PATCH="$HARNESS_DIR/bad-then-good-recovery-pass-shoreside.xmoos" ;;
        *) return 1 ;;
    esac
}
