...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H01-fixedturn_behavior_motion
...
Examples:
  ./$ME
  ./$ME --log=full
  ./$ME --case=port_90_pass
  ./$ME --jobs=4 --port_base=15000 --port_stride=12
  ./$ME --just_make --case=bad_fix_turn_fail
EOF
}
...
get_case_config() {
    ...
    if [ "$CASE_NAME" = "starboard_90_pass" ]; then
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/port-90-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/port-90-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "starboard_360_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/long-turn-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/starboard-360-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "port_360_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/long-port-turn-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/port-360-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "speed_auto_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/speed-auto-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/speed-auto-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "fixed_speed_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/fixed-speed-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/fixed-speed-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "turn_delay_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/turn-delay-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/turn-delay-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/turn-delay-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "timeout_complete_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/timeout-complete-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/timeout-complete-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "turn_spec_sequence_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/turn-spec-sequence-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/turn-spec-sequence-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "turn_spec_fallback_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/turn-spec-fallback-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/turn-spec-fallback-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "turn_spec_key_update_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/turn-spec-key-update-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/turn-spec-key-update-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/turn-spec-key-update-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "turn_spec_bad_update_recover_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/turn-spec-bad-update-recover-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/turn-spec-bad-update-recover-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/turn-spec-bad-update-recover-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "turn_spec_clear_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/turn-spec-clear-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/turn-spec-clear-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/turn-spec-clear-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "turn_spec_timeout_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/turn-spec-timeout-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/turn-spec-timeout-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "zero_fix_turn_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/zero-fix-turn-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/zero-fix-turn-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "runtime_static_update_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/runtime-static-update-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/runtime-static-update-pass-vehicle.xmoos"
    elif [ "$CASE_NAME" = "turn_spec_clear_add_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/turn-spec-clear-add-pass-shoreside.xmoos"
        CASE_VEH_MOOS_PATCH="$HARNESS_DIR/turn-spec-clear-add-pass-vehicle.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/turn-spec-clear-add-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "turn_spec_aliases_pass" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/turn-spec-aliases-pass-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/turn-spec-aliases-pass-vehicle.xbhv"
    elif [ "$CASE_NAME" = "bad_fix_turn_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-quick-fail-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-fix-turn-fail-vehicle.xbhv"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-quick-fail-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-turn-dir-fail-vehicle.xbhv"
        ...
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-quick-fail-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-turn-spec-fail-vehicle.xbhv"
    elif [ "$CASE_NAME" = "bad_schedule_repeat_fail" ]; then
        CASE_SHORE_PATCH="$HARNESS_DIR/eval-quick-fail-shoreside.xmoos"
        CASE_VEH_BHV_PATCH="$HARNESS_DIR/bad-schedule-repeat-fail-vehicle.xbhv"
    else
