...
PEER_STEM="$MISSION_DIR/meta_peer.moos"
PEER_XFILE="$MISSION_DIR/meta_peer.moosx"
...
for ARGI; do
    ...
    if [ "${ARGI}" = "--help" -o "${ARGI}" = "-h" ]; then
        ...
        echo "Examples:"
        echo "  ./zlaunch.sh"
        echo "  ./zlaunch.sh --case=pshare_direct_route_pass"
        echo "  ./zlaunch.sh --jobs=4 --port_base=11000"
        exit 0
...
get_case_config() {
    ...
    if [ "$PEER_PATCH" = "" ]; then
        local peer_patch_name="${CASE_NAME//_/-}-peer.xmoos"
        if [ -f "$HARNESS_DIR/$peer_patch_name" ]; then
            PEER_PATCH="$HARNESS_DIR/$peer_patch_name"
        fi
    fi
    ...
    if [ "$PEER_PATCH" != "" ] && [ ! -f "$PEER_PATCH" ]; then
        echo "$ME: Missing case patch: [$PEER_PATCH]"
        return 1
    fi
...
apply_case_patches() {
    ...
    if [ "$PEER_PATCH" != "" ]; then
        nspatch --stem="$PEER_STEM" "$PEER_PATCH" --targ="$PEER_XFILE"
    fi
}
...
run_case() {
    ...
    if [ "$PORT_BASE_SET" = "yes" ]; then
        ...
        peer_pshare=$((case_base + 11))
        xargs="$xargs --shore_mport=$shore_mport --peer_mport=$peer_mport --shore_pshare=$shore_pshare --peer_pshare=$peer_pshare"
    fi
    if [ "$NOGUI" != "" ]; then
        xargs="$xargs $NOGUI"
    fi
...
prepare_case_dir() {
    ...
    if [ "$PEER_PATCH" != "" ]; then
        nspatch --stem="$case_dir/meta_peer.moos" "$PEER_PATCH" --targ="$case_dir/meta_peer.moosx"
    fi
}
...
run_case_isolated() {
    ...
    (
        cd "$case_dir"
        : > results.txt
        xargs="--max_time=$MAX_TIME --mmod=$case_name --shore_mport=$shore_mport --peer_mport=$peer_mport --shore_pshare=$shore_pshare --peer_pshare=$peer_pshare $TIME_WARP"
        if [ "$NOGUI" != "" ]; then
            xargs="$xargs $NOGUI"
        fi
        if [ "$JUST_MAKE" = "yes" ]; then
...
if [ "$JOBS" -le 1 ] || [ "$CASE" != "" ]; then
    ...
    RUN_ROOT=$(mktemp -d "$HARNESS_DIR/.parallel_pshare_unit_XXXXXX")
