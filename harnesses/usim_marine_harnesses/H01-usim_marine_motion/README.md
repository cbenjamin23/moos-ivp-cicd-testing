...
- `start_pos_seed_pass`: Configures `start_pos=x=20,y=-10,heading=180,speed=1,depth=3` and applies thrust `20`, testing all five configured start fields before southbound motion; at three seconds, passes with `18<=NAV_X<=22`, `NAV_Y<=-12`, heading in `[178,182]`, speed at least `0.8`, and depth in `[2.8,3.2]`.
...
- `disabled_nav_seed_pass`: Disables the simulator, posts `NAV_X=42`, `NAV_Y=-17`, and `NAV_HEADING=275`, then reenables it and applies thrust `20`; at four seconds, it passes only after simulator-owned motion moves west to `38<=NAV_X<=41` with `-18<=NAV_Y<=-15`, heading in `[274,276]`, and speed in `[0.8,1.2]`.
...
- `dual_state_reset_nav_pass`: Enables dual-state mode with `drift_x=1`, first requiring ordinary `NAV_X` near zero while `NAV_GT_X` has advanced at least three meters, then posts `USM_RESET_NAV=true` and stops the drift; at 3.5 seconds, it passes with both x states in `[3,8]`, y near zero, and speed near zero.
