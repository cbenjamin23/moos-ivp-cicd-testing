...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H01-usim_marine_motion
...
Examples:
  ./$ME
  ./$ME --log=full
  ./$ME --case=thrust_forward_pass
  ./$ME --jobs=2 --port_base=22000
  ./$ME --just_make --case=trim_control_report_pass
EOF
}
...
get_case_config() {
    ...
    case "$case_name" in
        thrust_forward_pass)
            ;;
        rudder_turn_starboard_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/rudder-turn-starboard-pass.xmoos"
            ;;
        differential_turn_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/differential-turn-pass.xmoos"
            ;;
        start_pos_seed_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/start-pos-seed-pass.xmoos"
            ;;
        max_speed_clamp_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/max-speed-clamp-pass.xmoos"
            ;;
        acceleration_limit_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/acceleration-limit-pass.xmoos"
            ;;
        drift_x_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/drift-x-pass.xmoos"
            ;;
        rotate_speed_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/rotate-speed-pass.xmoos"
            ;;
        sim_pause_holds_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/sim-pause-holds-pass.xmoos"
            ;;
        reset_count_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/reset-count-pass.xmoos"
            ;;
        disabled_nav_seed_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/disabled-nav-seed-pass.xmoos"
            ;;
        reverse_thrust_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/reverse-thrust-pass.xmoos"
            ;;
        elevator_dive_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/elevator-dive-pass.xmoos"
            ;;
        runtime_turn_rate_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/runtime-turn-rate-pass.xmoos"
            ;;
        obstacle_hit_stop_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/obstacle-hit-stop-pass.xmoos"
            ;;
        current_y_alias_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/current-y-alias-pass.xmoos"
            ;;
        drift_vector_mult_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/drift-vector-mult-pass.xmoos"
            ;;
        water_depth_altitude_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/water-depth-altitude-pass.xmoos"
            ;;
        thrust_map_interpolation_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/thrust-map-interpolation-pass.xmoos"
            ;;
        max_deceleration_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/max-deceleration-pass.xmoos"
            ;;
        embedded_pid_speed_factor_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/embedded-pid-speed-factor-pass.xmoos"
            ;;
        drift_vector_add_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/drift-vector-add-pass.xmoos"
            ;;
        runtime_buoyancy_rate_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/runtime-buoyancy-rate-pass.xmoos"
            ;;
        rudder_slew_limit_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/rudder-slew-limit-pass.xmoos"
            ;;
        turn_speed_map_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/turn-speed-map-pass.xmoos"
            ;;
        wind_polar_speed_cap_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/wind-polar-speed-cap-pass.xmoos"
            ;;
        dual_state_reset_nav_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/dual-state-reset-nav-pass.xmoos"
            ;;
        wormhole_transport_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/wormhole-transport-pass.xmoos"
            ;;
        runtime_wind_conditions_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/runtime-wind-conditions-pass.xmoos"
            ;;
        runtime_water_depth_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/runtime-water-depth-pass.xmoos"
            ;;
        runtime_thrust_mode_reverse_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/runtime-thrust-mode-reverse-pass.xmoos"
            ;;
        max_depth_rate_limit_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/max-depth-rate-limit-pass.xmoos"
            ;;
        positive_buoyancy_surface_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/positive-buoyancy-surface-pass.xmoos"
            ;;
        embedded_pid_depth_control_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/embedded-pid-depth-control-pass.xmoos"
            ;;
        buoyancy_control_report_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/buoyancy-control-report-pass.xmoos"
            ;;
        trim_control_report_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/trim-control-report-pass.xmoos"
            ;;
        *) return 1 ;;
    esac
}
