...
#------------------------------------------------------------
#   Script: zlaunch.sh
#  Harness: H01-stationkeep_behavior_motion
...
Examples:
  ./$ME
  ./$ME --case=point_update_retarget_pass
  ./$ME --jobs=4 --port_base=15000 --port_stride=12
  ./$ME --just_make --case=bad_point_fail
EOF
}
...
get_case_config() {
    ...
    case "$CASE_NAME" in
        ...
        wide_radius_pass)
            ...
            ;;
        tight_radius_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/tight-radius-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/tight-radius-pass-vehicle.xbhv"
            ;;
        inner_gt_outer_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/inner-gt-outer-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/inner-gt-outer-pass-vehicle.xbhv"
            ;;
        outer_speed_slow_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/outer-speed-slow-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/outer-speed-slow-pass-vehicle.xbhv"
            ;;
        outer_speed_update_slow_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/outer-speed-update-slow-pass-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/outer-speed-update-slow-pass-vehicle.xmoos"
            ;;
        transit_speed_fast_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/transit-speed-fast-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/transit-speed-fast-pass-vehicle.xbhv"
            ;;
        extra_speed_alias_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/extra-speed-alias-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/extra-speed-alias-pass-vehicle.xbhv"
            ;;
        transit_speed_slow_in_progress_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/transit-speed-slow-in-progress-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/transit-speed-slow-in-progress-pass-vehicle.xbhv"
            ;;
        hibernation_seek_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/hibernation-seek-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/hibernation-seek-pass-vehicle.xbhv"
            ;;
        hibernation_settle_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/hibernation-settle-pass-shoreside.xmoos"
            CASE_VEH_BHV_PATCH="$HARNESS_DIR/hibernation-settle-pass-vehicle.xbhv"
            ;;
        hibernation_radius_update_settle_pass)
        ...
        radius_update_expand_pass)
            ...
            ;;
        radius_update_shrink_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/radius-update-shrink-pass-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/radius-update-shrink-pass-vehicle.xmoos"
            ;;
        speed_update_slow_progress_pass)
            CASE_SHORE_PATCH="$HARNESS_DIR/speed-update-slow-progress-pass-shoreside.xmoos"
            CASE_VEH_MOOS_PATCH="$HARNESS_DIR/speed-update-slow-progress-pass-vehicle.xmoos"
            ;;
        bad_point_update_recover_pass)
