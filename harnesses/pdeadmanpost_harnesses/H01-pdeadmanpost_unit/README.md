...
Logging defaults to the twelve deadflag variables needed by the count checks
plus the evaluation cutoff. Use `--log=full` for the original diagnostic set.
...
- `stop_heartbeat_posts_pass`: Starts active with a 0.45-second timeout, sends heartbeats at 0.2 and 0.4, then stops, exercising stale detection after heartbeat loss; passes when `DEAD_STOP=true` is present by time 1.2.
...
- `reset_policy_reposts_after_heartbeat_pass`: Uses `post_policy=reset`, allows an initial trip, sends a heartbeat at 0.8, and goes stale again; passes when `DEAD_RESET=true` and at least two publications occur.
...
- `zero_max_noheart_posts_pass`: Sets `max_noheart=0` on an active watchdog, exercising the zero-timeout configuration; passes when `DEAD_ZERO_MAX=true` is published by time 0.8.
