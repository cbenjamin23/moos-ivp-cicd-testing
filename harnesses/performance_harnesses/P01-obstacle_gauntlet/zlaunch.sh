...
stop_root() {
    local root="$1"
    moos_scoped_teardown_stop_root "$root"
}
...
get_case_config() {
    ...
    case "$case_name" in
        baseline_field_pass)
            CASE_SCENARIO=baseline_field
            SHORE_PATCH="$HARNESS_DIR/baseline-field-pass-shoreside.xmoos"
            if [ "$PERF_PROFILE" = ci ]; then
                WALL_MIN=80.0; WALL_MAX=95.0
            else
                WALL_MIN=16.5; WALL_MAX=20.0
            fi
            ;;
        dense_field_pass)
            CASE_SCENARIO=dense_field
            ...
            ;;
        endurance_random_pass)
            CASE_SCENARIO=endurance_random
            SHORE_PATCH="$HARNESS_DIR/endurance-random-pass-shoreside.xmoos"
            CASE_MAX_TIME=2800
            if [ "$PERF_PROFILE" = ci ]; then
                WALL_MIN=780.0; WALL_MAX=900.0
            else
                WALL_MIN=145.0; WALL_MAX=180.0
            fi
            ;;
        *) return 1 ;;
    esac
...
highest_port=$((PORT_BASE + (${#RUN_CASES[@]} - 1) * PORT_STRIDE + PSHARE_OFFSET + 1))
...
if [ "$RESULT_COUNT" -ne "${#RUN_CASES[@]}" ] || \
   { [ "${#RUN_CASES[@]}" -gt 0 ] && [ "$RESULT_COUNT" -eq 0 ]; }; then
