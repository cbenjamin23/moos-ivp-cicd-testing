...
Options:
  ...
  --veh_mport=<n>      First vehicle MOOSDB port
  --shore_pshare=<n>   Shoreside pShare port
  --veh_pshare=<n>     First vehicle pShare port
  --min_util_cpa=<n>   Minimum utility CPA distance
  --max_util_cpa=<n>   Maximum utility CPA distance
EOF
}
...
for arg in "$@"; do
    case "$arg" in
        ...
        --mmod=*|--shore_mport=*|--veh_mport=*|--shore_pshare=*|--veh_pshare=*|--min_util_cpa=*|--max_util_cpa=*)
            FLOW_ARGS+=("$arg")
            ;;
        *)
...
if [ "$JUST_MAKE" = yes ]; then
    if [ "$launch_rc" -ne 0 ] || [ ! -s targ_shoreside.moos ] || \
       ...
       [ ! -s targ_ben.moos ] || [ ! -s targ_ben.bhv ]; then
        echo "$ME: target generation did not produce the expected COLREGS files" >&2
        exit 1
    fi
    exit 0
fi

# pMissionEval may finish its report write just after xlaunch observes the
# mission-complete flag. Give that existing write a short bounded grace.
