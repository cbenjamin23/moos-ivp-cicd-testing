...
Options:
  ...
  --alpha_mport=<n>    Alpha MOOSDB port
  --bravo_mport=<n>    Bravo MOOSDB port
  --relay_mport=<n>    Relay MOOSDB port
  --shore_pshare=<n>   Shoreside pShare port
  --alpha_pshare=<n>   Alpha pShare port
  --bravo_pshare=<n>   Bravo pShare port
  --relay_pshare=<n>   Relay pShare port
...
for arg in "$@"; do
    case "$arg" in
        ...
        --shore_mport=*|--alpha_mport=*|--bravo_mport=*|--relay_mport=*|--shore_pshare=*|--alpha_pshare=*|--bravo_pshare=*|--relay_pshare=*|--mmod=*) FLOW_ARGS+=("$arg") ;;
        *)
...
[ "$JUST_MAKE" = yes ] && rm -f targ_shoreside.moos targ_alpha.moos targ_bravo.moos targ_relay.moos
...
if [ "$JUST_MAKE" = yes ]; then
    if [ "$launch_rc" -ne 0 ] || [ ! -s targ_shoreside.moos ] ||
       [ ! -s targ_alpha.moos ] || [ ! -s targ_bravo.moos ] || [ ! -s targ_relay.moos ]; then
        echo "$ME: target generation did not produce all four topology target files" >&2
        exit 1
    fi
    exit 0
fi
