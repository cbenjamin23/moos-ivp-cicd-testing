...
nsplug "$SHORE_MOOS" targ_shoreside.moos $NSFLAGS \
       ...
       MMOD=${MMOD:=pantler_baseline_launch_pass}
...
vecho "Launching pantler community"
if [ "$MMOD" = "pantler_filter_launch_pass" ]; then
    pAntler targ_shoreside.moos "MOOSDB pRealm pLogger uTimerAllowed pMissionEval pAutoPoke pMissionHash" >& /dev/null &
else
