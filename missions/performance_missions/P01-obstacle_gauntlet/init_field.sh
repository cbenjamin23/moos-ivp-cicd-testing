...
for ARGI; do
    if [ "${ARGI}" = "--help" -o "${ARGI}" = "-h" ]; then
        ...
        echo "  --mmod=<mod>       baseline_field_pass | dense_field_pass"
...
vecho "Preparing vehicle field files for [$MMOD]"
...
case "$MMOD" in
    ...
    endurance_random_pass)
        ...
        if [ $? != 0 ]; then
            echo "$ME Unable to Gen Obstacles for [$MMOD]. Exit code 2."
            exit 2
        fi
        ;;
    *)
        echo "$ME: Unknown mission mode: [$MMOD]. Exit Code 2."
        exit 2
        ;;
esac

if [ "$MMOD" != "endurance_random_pass" ] && [ ! -f obstacles.txt ]; then
    echo "$ME: Unable to stage fixed obstacle file for [$MMOD]. Exit Code 3."
    exit 3
fi
