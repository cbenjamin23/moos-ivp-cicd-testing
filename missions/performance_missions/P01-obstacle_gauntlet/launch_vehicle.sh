...
for ARGI; do
    ...
    if [ "${ARGI}" = "--help" -o "${ARGI}" = "-h" ]; then
        ...
        echo "  --sim, -s              Sim, not field vehicle"
        ...
        echo "  --model_rad=<dist>     Dubins turn radius"
        exit 0
    ...
    elif [ "${ARGI:0:12}" = "--model_rad=" ]; then
        MODEL_RAD="${ARGI#--model_rad=*}"
    else
