...
Options:
  ...
  --scenario=<name>    baseline_circle | mixed_speed_circle |
                       endurance_circle | noncoop_circle
...
case "$SCENARIO" in
    baseline_circle|mixed_speed_circle|noncoop_circle)
        [ -n "$MAX_TIME" ] || MAX_TIME=700
        ;;
    endurance_circle) [ -n "$MAX_TIME" ] || MAX_TIME=1400 ;;
...
if [ "$JUST_MAKE" = yes ]; then
    ...
    if [ "$launch_rc" -ne 0 ] || [ ! -s targ_shoreside.moos ] || \
       [ "$vehicle_moos" -ne 5 ] || [ "$vehicle_bhv" -ne 5 ]; then
