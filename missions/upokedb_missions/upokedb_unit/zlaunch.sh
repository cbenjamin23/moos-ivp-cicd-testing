...
Options:
  ...
  --external_stimulus   Let a harness invoke uPokeDB after startup
EOF
}
...
if [ "$JUST_MAKE" = yes ]; then
    ...
    if [ "$EXTERNAL_STIMULUS" = no ]; then
        sleep 0.75
        uPokeDB --quiet --host=localhost \
            --port="$MOOS_PORT" \
            POKE_NUM=42.5 TEST_EVAL_READY=true > upokedb.out 2>&1 || stimulus_rc=$?
    fi
