...
#  Mission: memoryturnlimit_behavior_motion
...
for ARGI; do
    ...
    if [ "${ARGI}" = "--help" -o "${ARGI}" = "-h" ]; then
        ...
        echo "  --nogui, -n        Headless mode, no GUI      "
        echo "  --ip=<localhost>   Force pHostInfo IP         "
        echo "  --mport=<9000>     Shoreside MOOSDB port      "
        echo "  --pshare=<9200>    Shoreside pShare port      "
        echo "  --mmod=<mod>       Mission variation/mod      "
        echo "  --vnames=<vnames>  Colon-separated vehicles   "
        exit 0
